...
class FairnessDataset():
    def __init__(self, dataset, device=torch.device('cuda'), X_train=None, y_train=None, X_test=None, y_test=None, sens_attrs=None):
        ...
        if self.dataset == 'AdultCensus':
            ...
            self.get_custom_data(X_train, y_train, X_test, y_test, sens_attrs)
    ...
    def get_custom_data(self, X_train, y_train, X_test, y_test, sens_attrs):
        self.Z_train_ = X_train[sens_attrs[0]]
        self.X_train_ = X_train.drop(labels=[sens_attrs[0]], axis=1)
        self.Y_train_ = y_train

        self.Z_test_ = X_test[sens_attrs[0]]
        self.X_test_ = X_test.drop(labels=[sens_attrs[0]], axis=1)
        self.Y_test_ = y_test
