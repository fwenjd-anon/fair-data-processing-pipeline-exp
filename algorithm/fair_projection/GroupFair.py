...
class GFair:
    ...
    def project(self,X,y=None,s=None,constraints=[('meo',.1)],rho=2,max_iter=1000,use_y = False,method='tf'):
        ...
        if method == 'tf':
            self.l = admm_tf(G, np.expand_dims(Py_x,axis=2),rho=rho,max_iter=max_iter,div=self.div)
        elif method == 'np':
            self.l = admm(G, np.expand_dims(Py_x, axis=2), rho=rho, max_iter=max_iter, report=True, div=self.div)
        else:
    ...
    def predict_proba(self,X,y=None,s=None):
        ...
        return predict(self.l, G, np.expand_dims(Py_x,axis=2),div=self.div)
