...
from .it import independence, balance, neg_accuracy
#import config
...
class CustomLogisticRegression:
    ...
    def custom_loss(self, weights):
        ...
        #print(list(self.Y).shape)
        
        loss = log_loss(self.Y, y_prob)
        if self.alpha != 0:
            loss += self.alpha * independence(list(self.Y), y_pred, self.A)
        if self.beta != 0:
            loss += self.beta * balance(list(self.Y), y_pred, self.A)
        if self.gamma != 0:
            loss += self.gamma * neg_accuracy(list(self.Y), y_pred)
        if self._lambda != 0:
